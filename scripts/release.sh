#!/bin/bash
# Build release archives and attach them to an already-gated draft release.
...
# for the given tag. This script deliberately cannot create or publish a
# release: only release-assets.yml may make the draft public after verifying
# the immutable Core/updater pair manifest.
...
RELEASE_JSON="$(gh release view "${VERSION}" --repo "${REPO}" --json isDraft,assets)" || {
    echo "Release ${VERSION} does not exist. Create it through release.yml; local publishing is disabled." >&2
    exit 1
}
if [ "$(jq -r .isDraft <<<"${RELEASE_JSON}")" != "true" ]; then
    echo "Release ${VERSION} is not a draft; refusing local mutation." >&2
    exit 1
fi
if ! jq -e '.assets | any(.name == "ftw-control-plane.json")' <<<"${RELEASE_JSON}" >/dev/null; then
    echo "Draft ${VERSION} lacks the verified ftw-control-plane.json gate." >&2
    exit 1
fi
...
echo "Assets attached to draft. Only release-assets.yml may publish it."
