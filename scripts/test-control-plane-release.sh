...
jq '.components.optimizer = {
  image: "ghcr.io/srcfl/ftw-optimizer:v1.3.2",
  digest: "sha256:cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc"
}' "${TMP}/manifest.json" >"${TMP}/manifest-with-optimizer.json"
if PATH="${TMP}/bin:${PATH}" scripts/verify-control-plane-manifest.sh \
  "${TMP}/manifest-with-optimizer.json" v1.4.0 \
  0123456789abcdef0123456789abcdef01234567 >/dev/null 2>&1; then
  echo "optimizer must stay outside the control-plane manifest" >&2
  exit 1
fi
