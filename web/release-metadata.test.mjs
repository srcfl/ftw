...
describe("release metadata", () => {
  ...
  it("keeps stable draft-only until the control-plane pair passes", () => {
    ...
    assert.match(releaseAssetsWorkflow, /verify-control-plane-manifest\.sh[\s\S]+gh release edit "\$\{TAG\}"[\s\S]+--draft=false/);
    assert.match(releaseAssetsWorkflow, /needs: \[meta, control-plane\]/);
  });

  it("checks the same-release Core and updater digests", () => {
  ...
  it("does not let the local helper create or publish releases", () => {
