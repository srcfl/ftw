...
#   make release              — Linux arm64/amd64 archives
#   make run-sim              — start both simulators locally
#   make dev                  — start sims + main app (hot-reload workflow)
#   make clean                — remove all build artifacts

.PHONY: help test compose-migration-test container-boundary-test release-workflow-test build build-arm64 build-amd64 release release-linux release-linux-arm64 release-linux-amd64 \
...
help:
	...
	@echo "  verify-all           pre-push: verify + Linux builds"
...
#
# verify-all checks both supported Linux targets on the developer host.
...
verify-all: verify
	...
	@echo "verify-all: local checks and Linux builds clean"
...
release-linux-arm64: drivers-present build-arm64
	python3 scripts/package-linux.py arm64

release-linux-amd64: drivers-present build-amd64
	python3 scripts/package-linux.py amd64
