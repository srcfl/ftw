...
func TestHistoryParquetResumesCommittedChunkAndRejectsChangedSource(t *testing.T) {
	...
	if _, err := conn.ExecContext(ctx, `CREATE TEMP TABLE history_import_source (ts_ms BIGINT NOT NULL,driver VARCHAR NOT NULL,metric VARCHAR NOT NULL,value DOUBLE NOT NULL,PRIMARY KEY(driver,metric,ts_ms))`); err != nil {
		t.Fatal(err)
	}
