...
func (s *Store) archiveBucketDay(ctx context.Context, cold string, day time.Time, cutoff int64) error {
	...
	flush := func() error {
		...
		err := stageBuckets(ctx, stage, batch, false)
		batch = batch[:0]
		if err != nil {
			return err
		}
	...
	from, to := day.UnixMilli(), min(day.Add(24*time.Hour).UnixMilli(), cutoff)
	rows, err := s.history.QueryContext(ctx, `SELECT d.name,m.name,b.start_ms,b.resolution_ms,b.first_ms,b.last_ms,b.n,b.sum_value,b.min_value,b.max_value,b.last_value FROM ts_buckets b JOIN ts_drivers d ON d.id=b.driver_id JOIN ts_metrics m ON m.id=b.metric_id WHERE b.resolution_ms=? AND b.start_ms>=? AND b.start_ms<? ORDER BY b.start_ms,d.name,m.name`, ArchiveResolutionMS, from, to)
	if err != nil {
		return err
	}
	for rows.Next() {
	...
	err = publishBucketStage(ctx, stage, path)
	if err != nil {
		return err
	}
	hash, err := historyFileHashContext(ctx, path)
	if err != nil {
		return err
	}
	// Seal before pruning. New Core emissions cannot backdate into a sealed
	// day; older raw files have their own independent import/correction path.
	if err := s.writeArchiveBatch(ctx, func(ctx context.Context, tx *sql.Tx) error {
		_, err := tx.ExecContext(ctx, `INSERT INTO ts_bucket_days(day_ms,sha256,resolution_ms,through_ms) VALUES(?,?,?,?) ON CONFLICT(day_ms) DO UPDATE SET sha256=excluded.sha256,resolution_ms=excluded.resolution_ms,through_ms=MAX(through_ms,excluded.through_ms)`, from, hash, ArchiveResolutionMS, to)
		return err
	}); err != nil {
		return err
	}

	pending := make([]metricBucket, 0, 64)
	drain := func() error {
		if len(pending) == 0 {
			return nil
		}
		err := s.pruneBucketMinutes(ctx, pending)
		pending = pending[:0]
		return err
	}
	if err := scanStagedBuckets(ctx, stage, func(b metricBucket) error {
