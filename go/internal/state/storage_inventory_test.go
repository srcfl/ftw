...
func TestSQLiteInventoryRequiresRuntimeStore(t *testing.T) {
	if _, err := (&Store{}).SQLiteInventory(context.Background()); err == nil {
		t.Fatal("SQLiteInventory succeeded without a runtime database")
	}
}
