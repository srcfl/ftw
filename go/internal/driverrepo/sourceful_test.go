...
`)
	...
	if !readOnly {
		...
		defaultMode = map[string]any{
			"strategy": "vendor_autonomous", "entrypoint": "driver_default_mode",
	...
	packagePayload := map[string]any{
		...
		"compatibility": []any{map[string]any{
			...
			"host": map[string]any{
				"product": sourcefulFTWHostProduct, "min_version": "1.4.0", "max_version_exclusive": "2.0.0",
			},
			"runtime": map[string]any{
				"name": sourcefulFTWRuntime, "semantics": sourcefulFTWSemantics,
				"version": sourcefulFTWRuntimeVersion, "abi": sourcefulFTWABI,
				"host_api": map[string]any{"profile": sourcefulFTWHostAPIProfile, "min": 1, "max": 1},
			},
			"control_enabled": !readOnly,
		}},
...
func TestSourcefulIndexPackageInstallAndOfflineCache(t *testing.T) {
	...
	fixture.build(t, server.URL, true)
	...
	if driver.ID != "sdm630" || !driver.Metadata.ReadOnly || driver.PackageID != "com.sourceful.driver.sdm630" ||
		...
		driver.RuntimeABI != sourcefulFTWABI || driver.HostAPIProfile != sourcefulFTWHostAPIProfile ||
...
func TestSourcefulPilotRejectsControlAndAmbiguousJSON(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(fixture.serveHTTP))
	defer server.Close()
	fixture.build(t, server.URL, false)
	cfg := &config.DeviceRepository{Enabled: true, Repositories: []config.DriverRepositorySource{{
		ID: "sourceful", Format: config.DriverRepositoryFormatSourcefulIndexV1,
		ManifestURL: server.URL + "/index.json", Enabled: true, AllowInsecure: true,
		TrustedKeys: map[string]string{"sourceful-test-1": base64.StdEncoding.EncodeToString(public)},
	}}}
	manager := NewWithHostVersion(cfg, t.TempDir(), nil, "1.4.0")
	if err := manager.Refresh(context.Background(), "sourceful"); err == nil || !strings.Contains(err.Error(), "read-only") {
		t.Fatalf("control package error = %v", err)
	}
}
