...
func TestExpectedWhScalesWithKWp(t *testing.T) {
	hours := summerHours()
	e5 := ExpectedWh(59.33, 18.07, []Array{{KWp: 5, TiltDeg: 35, AzimuthDeg: 180}}, hours)
	e10 := ExpectedWh(59.33, 18.07, []Array{{KWp: 10, TiltDeg: 35, AzimuthDeg: 180}}, hours)
	if e5 <= 0 {
		t.Fatalf("expected positive energy, got %.1f", e5)
	}
	if math.Abs(e10/e5-2) > 1e-9 {
		t.Errorf("10 kWp should be 2× 5 kWp, ratio %.4f", e10/e5)
	}
}

func TestExpectedWhSumsArrays(t *testing.T) {
	hours := summerHours()
	south := ExpectedWh(59.33, 18.07, []Array{{KWp: 5, TiltDeg: 35, AzimuthDeg: 180}}, hours)
	east := ExpectedWh(59.33, 18.07, []Array{{KWp: 5, TiltDeg: 35, AzimuthDeg: 90}}, hours)
	both := ExpectedWh(59.33, 18.07, []Array{
		{KWp: 5, TiltDeg: 35, AzimuthDeg: 180},
		{KWp: 5, TiltDeg: 35, AzimuthDeg: 90},
	}, hours)
...
func TestExpectedWhUsesMeasuredDiffuse(t *testing.T) {
	...
	arr := []Array{{KWp: 10, TiltDeg: 35, AzimuthDeg: 180}}
...
func TestExpectedWhZeroWhenDark(t *testing.T) {
	...
	if e := ExpectedWh(59.33, 18.07, []Array{{KWp: 10, TiltDeg: 35, AzimuthDeg: 180}}, night); e != 0 {
