...
func TestFromConfigGating(t *testing.T) {
	...
	svc := FromConfig(&config.Weather{
		...
		PVArrays: []config.PVArray{
			{KWp: 5, TiltDeg: f64(35), AzimuthDeg: f64(180)},
			{KWp: 3, TiltDeg: f64(20), AzimuthDeg: f64(90)},
		},
	}, 0, st, "ua")
	if svc == nil || len(svc.Arrays) != 2 {
		t.Fatalf("expected 2 arrays, got %+v", svc)
	}
	if svc.Arrays[0].KWp != 5 || svc.Arrays[1].AzimuthDeg != 90 {
	...
	if flat.Arrays[0].KWp != 8 || flat.Arrays[0].TiltDeg != 30 {
...
func TestScoreDayPersistsExpectedVsActual(t *testing.T) {
	...
	svc := &Service{
		...
		Lat:    59.33,
		Lon:    18.07,
		Arrays: []Array{{KWp: 10, TiltDeg: 35, AzimuthDeg: 180}},
	}
...
func TestScoreDaySkipsWhenNoHistory(t *testing.T) {
	st := openStore(t)
	svc := &Service{Store: st, Lat: 59.33, Lon: 18.07, Arrays: []Array{{KWp: 10, TiltDeg: 35, AzimuthDeg: 180}}}
