...
func FromConfig(cfg *config.Weather, ratedPVW float64, st *state.Store, userAgent string) *Service {
	...
	for _, a := range cfg.PVArrays {
		...
		arrays = append(arrays, Array{KWp: kWp, TiltDeg: tiltDeg, AzimuthDeg: azimuthDeg})
	}
	if len(arrays) == 0 && ratedPVW > 0 {
		arrays = append(arrays, Array{KWp: ratedPVW / 1000.0, TiltDeg: cfg.PVTiltDeg, AzimuthDeg: cfg.PVAzimuthDeg})
	}
	if len(arrays) == 0 {
		return nil
	}
