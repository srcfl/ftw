...
| Mode | Execution | Why |
|---|---|---|
| `planner_self` | Reactive self-consumption + per-slot gates derived from `SlotDirective.BatteryEnergyWh` and `PlannedGridW`. | The mode's contract is "never grid-charge and never export via the battery". Participant slots may discharge to cover local import; idle slots are charge-only and may absorb unexpected live meter surplus. If the plan explicitly forecasts PV export while battery action is idle, runtime holds battery at 0 so the export is preserved. See issue #130 + `docs/plan-ems-contract.md` §"Exception: planner_self". |
