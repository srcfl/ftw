...
func pairReleaseServer(t *testing.T, tag string, fixture *pairReleaseFixture) *httptest.Server {
	...
	srv = httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case r.URL.Path == "/asset":
			if fixture.extraOptimizer {
				data, _ := json.Marshal(fixture.manifest)
				var document map[string]any
				_ = json.Unmarshal(data, &document)
				document["components"].(map[string]any)["optimizer"] = map[string]string{
					"image": "ghcr.io/srcfl/ftw-optimizer:v1.3.2", "digest": testUpdaterDigest,
				}
...
func pairChecker(srv *httptest.Server) *Checker {
	return New(Config{
...
func TestControlPlaneReleaseRequiresBothPinnedDigests(t *testing.T) {
	...
	info, err := pairChecker(srv).Check(context.Background(), true)
	if err != nil {
		t.Fatal(err)
	}
...
func TestControlPlaneReleaseRejectsMovedTagDigest(t *testing.T) {
	...
	srv := pairReleaseServer(t, tag, &fixture)
	_, err := pairChecker(srv).Check(context.Background(), true)
...
func TestControlPlaneReleaseRejectsOptimizerComponent(t *testing.T) {
	const tag = "v1.4.0"
	fixture := newPairReleaseFixture(tag)
	fixture.extraOptimizer = true
	srv := pairReleaseServer(t, tag, &fixture)
	_, err := pairChecker(srv).Check(context.Background(), true)
	if err == nil || !strings.Contains(err.Error(), `unknown field "optimizer"`) {
		t.Fatalf("Check error = %v", err)
	}
}
