...
// unknown fields and recreated Core only. This fixture deliberately mirrors
// that wire contract. New Core must reject it before POST and, together with
// the boot-phase 503 readiness gate, therefore cannot migrate in a mixed pair.
func TestV131UpdaterIsRejectedBeforeCoreUpdate(t *testing.T) {
	...
	c := New(Config{SocketPath: socket, UpdaterVersion: "v1.4.0"}, newMemStore())
	if c.Info().SidecarReady {
		t.Fatal("v1.3.1 updater must not be actionable for a new Core")
	}
