...
- **Pi** on the home LAN at `192.168.192.40` (running `FTW`).
- **Cloud relay** `relay.ftw.sourceful.energy` (running `ftw-relay`).
- **DNS/TLS:** `sourceful.energy` for the relay, plus the existing
  `fortytwowatts.com` zone for the stable home/WebAuthn origin.
...
> written for the Phases 1–3 throwaway deploy under `relay.ftw.sourceful.energy`.
> That cutover has since **shipped in code**: the owner WebAuthn RP-ID now
> defaults to **`home.fortytwowatts.com`** (`go/cmd/ftw/main.go`),
...
> to `relay.ftw.sourceful.energy`**, or every passkey you enrol is bound to a
...
> **Current migration state:** code defaults now point to
> `relay.ftw.sourceful.energy`, but relay/TURN provisioning may lag this
> repository migration. Do not use this runbook as evidence that the endpoint
> is live; verify DNS, TLS, `/healthz`, ICE and an off-LAN connection first.
...
`go/cmd/ftw/main.go:1491`). If `site.name` is `Home`, every
...
## 1. Cloudflare — DNS for `relay.ftw.sourceful.energy`

Create the relay record in `sourceful.energy`. Keep the existing home record in
`fortytwowatts.com` because that hostname is still the WebAuthn RP ID:

| Zone | Type | Name | Content | Proxy status |
|---|---|---|---|---|
| `sourceful.energy` | `A` | `relay.ftw` | `<RELAY_PUBLIC_IP>` | DNS-only for first bring-up |
| `fortytwowatts.com` | `A` | `home` | `<RELAY_PUBLIC_IP>` | Preserve current production policy |

That yields `relay.ftw.sourceful.energy → <RELAY_PUBLIC_IP>`. The separate
`home.fortytwowatts.com` record remains the owner-access origin.
...
- **Option B (proxied):** use the **Cloudflare Origin Cert** for
  `relay.ftw.sourceful.energy` (CF dashboard → SSL/TLS → Origin Server →
...
- **Option A (DNS-only):** the CF Origin Cert is **not** browser-trusted,
  ...
  sudo certbot certonly --standalone -d relay.ftw.sourceful.energy
  # → /etc/letsencrypt/live/relay.ftw.sourceful.energy/{fullchain,privkey}.pem
  ```
...
### 2.4 What the relay serves for `relay.ftw.sourceful.energy`
...
```bash
curl -sS https://relay.ftw.sourceful.energy/healthz        # → OK
# Before the Pi registers, this site is unknown:
curl -sS -o /dev/null -w '%{http_code}\n' \
  https://relay.ftw.sourceful.energy/me/site:<YourSiteName> # → 503 (site not registered)
```
...
Set these for the `FTW` process on the Pi. Defaults come from
`go/cmd/ftw/main.go` (`OwnerAccessRPID` now defaults to
...
`relay.ftw.sourceful.energy`); **the RP-ID/origins are the home HOST the browser
visits** (`home.fortytwowatts.com`) — these are different and must not be
conflated:

```ini
FTW_RELAY_URL=https://relay.ftw.sourceful.energy
...
make build-arm64
# → bin/ftw-linux-arm64  (CGO_ENABLED=0, static)

scp bin/ftw-linux-arm64 \
  <PI_USER>@192.168.192.40:/tmp/ftw
```
...
sudo install -m0755 /tmp/ftw /usr/local/bin/ftw
# Put the four env vars in the unit's EnvironmentFile, e.g.
#   /etc/ftw/owner-access.env
...
```ini
# in the [Service] block of ftw.service:
EnvironmentFile=/etc/ftw/owner-access.env
```
...
```bash
curl -sS -o /dev/null -w '%{http_code}\n' \
  https://relay.ftw.sourceful.energy/me/site:<YourSiteName>
...
A passkey usable at `https://relay.ftw.sourceful.energy` **must be created
with origin `https://relay.ftw.sourceful.energy`** — WebAuthn refuses to run
...
ceremony origin is correctly `relay.ftw.sourceful.energy`).
...
`go/cmd/ftw/owner_relay_register.go`) stamps the secret on
...
1. **RP-ID origin requirement.** The passkey must be born at origin
   `https://relay.ftw.sourceful.energy` (so it works there forever). →
...
1. **On the home LAN**, open `http://192.168.192.40:8080/` and fetch the
   ...
   ```bash
   journalctl -u ftw | grep 'LAN enrollment PIN' | tail -1
   ```
2. **From any browser** (phone or laptop, on or off the home WiFi) open:
   ```
   https://home.fortytwowatts.com/owner-access/enroll.html
   ```
   (Legacy phase-1–3 path: `https://relay.ftw.sourceful.energy/me/site:<YourSiteName>/owner-access/enroll.html`.)
...
2. Open:
   ```
   https://relay.ftw.sourceful.energy/me/site:<YourSiteName>/owner-access/login.html
   ```
...
curl -sS -X POST https://relay.ftw.sourceful.energy/me/register \
...
curl -sS -o /dev/null -w 'landing: %{http_code}\n' \
  https://relay.ftw.sourceful.energy/me/site:<YourSiteName>    # → 200
...
curl -sS -o /dev/null -w 'gate(/): %{http_code} %{redirect_url}\n' \
  https://relay.ftw.sourceful.energy/me/site:<YourSiteName>/      # → 302 → login.html (NOT 200 dashboard)
curl -sS -o /dev/null -w 'gate(api): %{http_code}\n' \
  https://relay.ftw.sourceful.energy/me/site:<YourSiteName>/api/status  # → 401/302, never 200

# (d) PIN endpoint MUST be LAN-only: through the relay it is forbidden.
curl -sS -o /dev/null -w 'pin-remote: %{http_code}\n' \
  https://relay.ftw.sourceful.energy/me/site:<YourSiteName>/api/owner-access/enroll-pin  # → 403
...
`ftw-linux-arm64`, or use the `switching-ftw-deploy-mode`
...
- **RP-ID must match the URL host exactly.** Passkeys enrolled under
  `relay.ftw.sourceful.energy` only work at `https://relay.ftw.sourceful.energy`.
...
| Thing | Value / path |
|---|---|
| Relay public URL | `https://relay.ftw.sourceful.energy` |
...
| Enroll (home host) | `https://home.fortytwowatts.com/owner-access/enroll.html` (legacy: `…relay.ftw.sourceful.energy/me/site:<YourSiteName>/owner-access/enroll.html`) |
...
| Disable remote access | unset `FTW_RELAY_URL`, restart FTW |

Source of truth: `go/cmd/ftw-relay/handlers.go` (relay routes),
`go/cmd/ftw/owner_relay_register.go` (Pi registration),
`go/cmd/ftw/main.go` (env wiring, `:1409`–`:1491`),
